...
def define_G(input_nc, output_nc, ngf, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02,
    ...
    net = ResnetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=[2, 2, 9])
...
class ResnetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False,
                 n_blocks=[2, 2, 9], padding_type='reflect'):
        assert (n_blocks[-1] >= 0)
        ...
        for i in range(n_blocks[0]):
            model += [ResBlock(ngf, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout,
                               use_bias=use_bias)]
        self.resblocks1 = nn.Sequential(*model)

        model = []
        for i in range(n_blocks[1]):
            model += [ResBlock(ngf * 2, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout,
                               use_bias=use_bias)]
    ...
    def forward(self, input):
        ...
        out['bottle'] = self.resblocks3(out['d2'])
        out['u1'] = self.up1(out['bottle'])
        # out['u2'] = self.up2(torch.cat([out['u1'], self.resblocks2(out['d1'])], axis=1))
        out['u2'] = self.up2(torch.cat([out['u1'], out['d1']], axis=1))

        # return self.outc(torch.cat([out['u2'], self.resblocks1(out['in'])], axis=1))
        return self.outc(torch.cat([out['u2'],out['in']], axis=1))
